...
2. Enter:
   ...
   - `Device name key` (optional): key path for device/camera name in payload
     (for example `camera`).
   - `Device name filter` (optional): comma-separated device names to accept
     (for example `Camera01, Camera02`).
...
`Device name key` only when set. When `Device name filter` has one or more
values, the integration creates one entity per configured device name under the
same Home Assistant device and only updates the matching entity for each
webhook payload. Other fields such as `time` are ignored.
